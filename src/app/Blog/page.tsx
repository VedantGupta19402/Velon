...
const BlogPage: React.FC = () => {
  ...
  const posts: BlogPost[] = [
    {
      ...
      excerpt:
        'In a world that never stops, brands that hesitate get left behind. Here is how to keep pace without losing your identity.',
      date: 'May 12, 2026',
    },
    {
      ...
      excerpt:
        'Static is dying. We explore how kinetic design systems are reshaping the way audiences experience brand identity.',
      date: 'Apr 28, 2026',
    },
    {
      ...
      excerpt:
        'Great work comes from great rhythm. A look inside how Velon structures creative teams for maximum flow.',
      date: 'Apr 10, 2026',
    },
    {
      ...
      excerpt:
        'The old campaign model is broken. We share how always-on creativity is replacing the once-a-year big idea.',
      date: 'Mar 22, 2026',
    },
    {
      ...
      excerpt:
        'More touchpoints are not the answer. We break down why speed and relevance beat scale every time.',
      date: 'Mar 5, 2026',
    },
    {
      ...
      excerpt:
        'An exploration of how animated typography is redefining storytelling in digital spaces.',
      date: 'Feb 18, 2026',
    },
  ];

  const categories: string[] = ['All', 'Strategy', 'Design', 'Culture', 'Advertising', 'Branding'];
  ...
  const handleTilt = useCallback((e: React.MouseEvent<HTMLDivElement>, index: number) => {
  ...
  const resetTilt = useCallback((index: number) => {
    const card = cardRefs.current[index];
    if (!card) return;
    const inner = card.querySelector('.tilt-inner');
    if (inner) {
  ...
  return (
    <div className='text-black bg-white'>
      <div className='pt-[28vh] pb-24 px-12 relative overflow-hidden'>
        ...
        />
        <div
          className='orbs w-[25vw] h-[25vw] bg-black top-[30vh] right-[-5vw] opacity-5'
      ...
      <div className='categories-wrap px-12 pb-16 flex justify-center gap-8 flex-wrap'>
        {categories.map((cat) => (
          <button
            key={cat}
            className='category-pill font-[font2] text-sm uppercase tracking-[0.15em] px-8 py-3 rounded-full border-2 border-black/20 hover:bg-black hover:text-white hover:border-black transition-all duration-300'
          >
            {cat}
          </button>
        ))}
      ...
      <div className='featured-wrap px-12 pb-24'>
        <div className='featured-card group cursor-pointer relative overflow-hidden rounded-[3vw] bg-black text-white'>
          ...
          <div className='absolute bottom-0 left-0 p-14 lg:p-24'>
            ...
            <span className='font-[font2] text-xs uppercase tracking-[0.1em] text-white/40 mt-6 block'>
              May 20, 2026
            </span>
          </div>
        </div>
          ...
          <div className='flex flex-col sm:flex-row justify-between items-center gap-8'>
            <div className='flex gap-8'>
              {['fb', 'ig', 'in', 'be'].map((s) => (
                <button
                  key={s}
                  className='font-[font2] uppercase border-4 border-white rounded-[3.5vw] px-8 sm:px-10 py-3 hover:text-[#D3FD50] hover:border-[#D3FD50] transition-all duration-300'
                >
                  {s}
                </button>
              ))}
            ...
            <a
              href='#top'
              className='font-[font2] uppercase border-4 border-white rounded-[3.5vw] px-8 sm:px-10 py-3 text-xl sm:text-2xl hover:text-[#D3FD50] hover:border-[#D3FD50] transition-all duration-300'
            >
              Back to top
            </a>
          </div>
